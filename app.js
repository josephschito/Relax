...
Opal.modules["relax"] = function(Opal) {/* Generated by Opal 1.7.3 */
  var $module = Opal.module, $klass = Opal.klass, $range = Opal.range, $def = Opal.def, $const_set = Opal.const_set, $to_ary = Opal.to_ary, $send = Opal.send, $send2 = Opal.send2, $find_super = Opal.find_super, $hash2 = Opal.hash2, $eqeq = Opal.eqeq, $rb_plus = Opal.rb_plus, $rb_minus = Opal.rb_minus, $nesting = [], nil = Opal.nil, $$$ = Opal.$$$;

  Opal.add_stubs('attr_accessor,rand,map,times,new,each,==,intensity,send,changing_color,[]=,directions,[],intensity=,+,-,r,g,b,a,css,update_color');
  return (function($base, $parent_nesting) {
    var self = $module($base, 'Relax');
    ...
    (function($base, $super) {
      ...
      self.$attr_accessor("intensity");
      return $def(self, '$initialize', function $$initialize() {
        var self = this;

        return (self.intensity = self.$rand($range(0, 255, false)))
      });
    })($nesting[0], null);
    (function($base, $super, $parent_nesting) {
      var self = $klass($base, $super, 'RGBAColor');

      var $nesting = [self].concat($parent_nesting), $$ = Opal.$r($nesting);

      
      self.$attr_accessor("r", "g", "b", "a", "percentage");
      $const_set($nesting[0], 'COLOR_PARAMETERS', ["r", "g", "b"]);
      return $def(self, '$initialize', function $$initialize() {
        var $a, $b, self = this;

        
        $b = $send((3).$times(), 'map', [], function $$1(){
          return $$('Color').$new()}), $a = $to_ary($b), (self.r = ($a[0] == null ? nil : $a[0])), (self.g = ($a[1] == null ? nil : $a[1])), (self.b = ($a[2] == null ? nil : $a[2])), $b;
        self.a = 1;
        return (self.percentage = 10);
      });
    })($nesting[0], $$('Color'), $nesting);
    (function($base, $super, $parent_nesting) {
      var self = $klass($base, $super, 'ChangingColor');

      var $nesting = [self].concat($parent_nesting), $$ = Opal.$r($nesting);

      
      self.$attr_accessor("directions");
      return $def(self, '$initialize', function $$initialize() {
        var $yield = $$initialize.$$p || nil, self = this;

        $$initialize.$$p = null;
        
        $send2(self, $find_super(self, 'initialize', $$initialize, false, true), 'initialize', [], $yield);
        self.directions = $hash2(["r", "g", "b"], {"r": "→", "g": "→", "b": "→"});
        return (self.rgba_color = $$('RGBAColor').$new());
      });
    })($nesting[0], $$('RGBAColor'), $nesting);
    return (function($base, $super, $parent_nesting) {
      var self = $klass($base, $super, 'Background');

      var $nesting = [self].concat($parent_nesting), $$ = Opal.$r($nesting);

      
      self.$attr_accessor("changing_color");
      
      $def(self, '$initialize', function $$initialize() {
        var self = this;

        return (self.changing_color = $$('ChangingColor').$new())
      });
      
      $def(self, '$update_color', function $$update_color() {
        var self = this;

        
        $send($$$($$('RGBAColor'), 'COLOR_PARAMETERS'), 'each', [], function $$2(color_parameter){var $a, self = $$2.$$s == null ? this : $$2.$$s, $binary_op_recvr_tmp_1 = nil, $binary_op_recvr_tmp_2 = nil;

          
          if (color_parameter == null) color_parameter = nil;
          if ($eqeq(self.$changing_color().$send(color_parameter).$intensity(), 0)) {
            self.$changing_color().$directions()['$[]='](color_parameter, "→")
          };
          if ($eqeq(self.$changing_color().$send(color_parameter).$intensity(), 255)) {
            self.$changing_color().$directions()['$[]='](color_parameter, "←")
          };
          if ($eqeq(self.$changing_color().$directions()['$[]'](color_parameter), "→")) {
            
            $binary_op_recvr_tmp_1 = self.$changing_color().$send(color_parameter);
            $binary_op_recvr_tmp_1['$intensity=']($rb_plus($binary_op_recvr_tmp_1.$intensity(), 1));
          };
          if ($eqeq(self.$changing_color().$directions()['$[]'](color_parameter), "←")) {
            
            $binary_op_recvr_tmp_2 = self.$changing_color().$send(color_parameter);
            return ($a = [$rb_minus($binary_op_recvr_tmp_2.$intensity(), 1)], $send($binary_op_recvr_tmp_2, 'intensity=', $a), $a[$a.length - 1]);
          } else {
            return nil
          };}, {$$s: self});
        return self;
      ...
      $def(self, '$css', function $$css() {
        var self = this, rgba_color = nil;

        
        rgba_color = self.$changing_color();
        return "rgba(" + (rgba_color.$r().$intensity()) + ", " + (rgba_color.$g().$intensity()) + ", " + (rgba_color.$b().$intensity()) + ", " + (rgba_color.$a()) + ")";
      });
      return $def(self, '$update', function $$update() {
        var self = this;

        return document.body.style.backgroundColor = self.$update_color().$css()
      });
    })($nesting[0], null, $nesting);
  })($nesting[0], $nesting)
};
...
Opal.queue(function(Opal) {/* Generated by Opal 1.7.3 */
  var self = Opal.top, $nesting = [], $$ = Opal.$r($nesting), nil = Opal.nil, $$$ = Opal.$$$, background = nil;

  Opal.add_stubs('require,include,new,update');
  
  self.$require("./relax.rb");
  self.$include($$('Relax'));
  background = $$$($$('Relax'), 'Background').$new();
  return setInterval(function(){background.$update()}, 50);
});
